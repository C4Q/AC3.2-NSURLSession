...
class InstaDogParser {
    ...
    func parseDogs(from apiEndpoint: String, callback: @escaping ([InstaDog]?) -> Void) {
        ...
        if let validInstaDogEndpoint: URL = URL(string: apiEndpoint) {
            ...
            session.dataTask(with: validInstaDogEndpoint) { (data: Data?, response: URLResponse?, error: Error?) in
                ...
                if let validData: Data = data {
                    ...
                    let allTheDogs: [InstaDog]? = InstaDogParser().getInstaDogs(from: validData)
                    callback(allTheDogs)
                }
